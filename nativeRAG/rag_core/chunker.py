...
class LegalStructureAwareChunker:
    """
    Bộ phân tách văn bản chuyên biệt cho văn bản quy phạm pháp luật định dạng Markdown.
    Tự động bóc tách: Tiêu đề văn bản, Số hiệu, Tình trạng hiệu lực, Chương, Mục, Điều.
    Gắn Context Header vào từng chunk để bảo toàn trọn vẹn ngữ cảnh khi truy xuất.
    """
    ...
    def parse_file(self, file_path: Path | str) -> List[LegalChunk]:
        path = Path(file_path)
        with open(path, "r", encoding="utf-8") as f:
            text = f.read()

        return self.parse_text(text, source_filename=path.name)

    def parse_text(self, text: str, source_filename: str = "") -> List[LegalChunk]:
        ...
        if h1_match:
            ...
            # Tìm số hiệu chuẩn (ví dụ 13/2023/NĐ-CP, 05/2026/TT-BKHCN, 116/2025/QH15, 127/QĐ-TTg)
            code_match = re.search(r"(\d+/\d+/[\wĐđ-]+|\d+/[\wĐđ-]+/[\wĐđ-]+|\d+/[\wĐđ-]+)", doc_title)
            if not code_match:
                code_match = re.search(r"(\d+-\d+-[\wĐđ-]+|\d+-[\wĐđ-]+-[\wĐđ-]+|\d+-[\wĐđ-]+)", doc_title)
            if code_match:
                raw_code = code_match.group(1)
                # Đổi các dấu gạch nối giữa số thành gạch chéo, vd: 13-2023-ND-CP -> 13/2023/ND-CP
        ...
        # Regex tìm trạng thái hiệu lực (thường ở dòng 2 hoặc 3)
        ...
        if not doc_code and source_filename:
            # Dự phòng lấy từ tên file
            code_from_file = source_filename.split("_")[0]
            doc_code = code_from_file.replace("-", "/")

        # 2. Duyệt qua từng dòng và gom theo cấu trúc Chương / Điều
        ...
        # 3. Chuyển đổi các section thành chunks kèm Context Prefix
        ...
        for sec in raw_sections:
            ...
            header_prefix = (
                f"[Văn bản: {doc_title or source_filename}]\n"
                f"[Số hiệu: {doc_code} | Tình trạng: {doc_status}]\n"
                f"[Phần: {chapter}]\n"
                f"[{article}]\n\n"
            )

            # Nếu nội dung nhỏ hơn kích thước chunk tối đa, giữ trọn vẹn Điều đó
            if len(header_prefix) + len(content) <= self.max_chunk_size:
                full_text = f"{header_prefix}{content}"
                chunk_id = f"{source_filename}_c{chunk_idx}"
                metadata = {
                ...
                chunks.append(LegalChunk(content=full_text, metadata=metadata))
                chunk_idx += 1
            else:
                # Nếu Điều quá dài, chia nhỏ theo đoạn/khoản nhưng vẫn đính kèm Header Prefix
    ...
    def _split_long_text(self, text: str, max_length: int) -> List[str]:
        """Tách văn bản dài theo dấu ngắt dòng đôi \n\n hoặc dòng đơn \n"""
        ...
        for p in paragraphs:
            if not current:
                ...
                # Gối đầu một phần nếu cần
                current = p

        if current:
            result.append(current)

        # Trường hợp vẫn còn đoạn văn quá dài (ít xảy ra)
        final_splits = []
        for item in result:
